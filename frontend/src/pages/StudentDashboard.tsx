...
const floatingBubbles = keyframes`
  ...
  100% {
    transform: translateY(-100vh) translateX(calc(20vw * var(--offset, 0))) scale(1);
    opacity: 0;
  }
`;
...
const glowingBorder = keyframes`
  0% {
    box-shadow: 0 0 5px rgba(79, 195, 247, 0.5), 0 0 10px rgba(79, 195, 247, 0.3);
  }
  50% {
    box-shadow: 0 0 10px rgba(79, 195, 247, 0.8), 0 0 20px rgba(79, 195, 247, 0.5);
  }
  100% {
    box-shadow: 0 0 5px rgba(79, 195, 247, 0.5), 0 0 10px rgba(79, 195, 247, 0.3);
  }
...
const PageContainer = styled(Box)(({ theme }) => ({
    ...
    minHeight: '100vh',
    background: `linear-gradient(-45deg, #FF9966, #FF5E62, #4ECDC4, #1A535C, #6A0572)`,
...
const StyledPaper = styled(Paper)(({ theme }) => ({
    ...
    background: `linear-gradient(135deg, ${alpha(theme.palette.background.paper, 0.85)} 0%, ${alpha(theme.palette.background.paper, 0.75)} 100%)`,
...
const Bubble = styled(Box)<BubbleProps>(({ theme, size = 30, offset = 0 }) => ({
    position: 'absolute',
    bottom: '-20px',
    ...
    height: `${size}px`,
    borderRadius: '50%',
    background: `radial-gradient(circle at 30% 30%, ${alpha(theme.palette.common.white, 0.9)}, ${alpha(theme.palette.primary.light, 0.4)})`,
    boxShadow: `0 0 10px ${alpha(theme.palette.primary.light, 0.5)}`,
    animation: `${floatingBubbles} ${6 + Math.random() * 6}s ease-in infinite`,
...
const StudentDashboard: React.FC = () => {
    ...
    const completedAssignments = assignments?.filter((a: any) => a.grade !== null)?.length || 0;
    const pendingAssignments = (assignments?.length || 0) - completedAssignments;
    const averageGrade = assignments?.reduce((acc: number, curr: any) => 
        curr.grade ? acc + curr.grade : acc, 0) / (completedAssignments || 1);
    ...
    return (
        <PageContainer>
            ...
            {/* İstatistik Kartları */}
            <Grid container spacing={5} mb={4} sx={{ px: 2, justifyContent: 'space-between' }}>
                <Grid sx={{ width: { xs: '100%', sm: '48%', md: '23%' } }}>
                    <StatCard>
                        <CardContent sx={{ position: 'relative', zIndex: 1, p: 3 }}>
                            ...
                            <StatLabel>
                                <Bookmarks fontSize="small" /> Toplam Ödev
                            </StatLabel>
                        </CardContent>
                    </StatCard>
                </Grid>
                <Grid sx={{ width: { xs: '100%', sm: '48%', md: '23%' } }}>
                    <StatCard>
                        <CardContent sx={{ position: 'relative', zIndex: 1, p: 3 }}>
                            <StatIcon className="stat-icon">
                                <Grading fontSize="inherit" />
                            </StatIcon>
                            <StatValue>
                                {completedAssignments}
                            </StatValue>
                            <StatLabel>
                                <TrendingUp fontSize="small" /> Tamamlanan
                            </StatLabel>
                        </CardContent>
                    </StatCard>
                </Grid>
                <Grid sx={{ width: { xs: '100%', sm: '48%', md: '23%' } }}>
                    <StatCard>
                        <CardContent sx={{ position: 'relative', zIndex: 1, p: 3 }}>
                            ...
                            <StatLabel>
                                <Assessment fontSize="small" /> Bekleyen
                            </StatLabel>
                        </CardContent>
                    </StatCard>
                </Grid>
                <Grid sx={{ width: { xs: '100%', sm: '48%', md: '23%' } }}>
                    <StatCard>
                        <CardContent sx={{ position: 'relative', zIndex: 1, p: 3 }}>
                            ...
                            <StatLabel>
                                <EmojiEvents fontSize="small" /> Ortalama Not
                            </StatLabel>
                        </CardContent>
                    </StatCard>
                </Grid>
            ...
            {/* Ana içerik - Ödevler ve Profil */}
            <Grid container spacing={3}>
                {/* Sol sütun - Profil kartı */}
                <Grid sx={{ gridColumn: { xs: 'span 12', md: 'span 4' }, order: { xs: 1, md: 1 } }}>
                    <ProfileCard elevation={3} sx={{ height: '100%' }}>
                ...
                {/* Sağ sütun - Ödevler listesi */}
                <Grid sx={{ gridColumn: { xs: 'span 12', md: 'span 8' }, order: { xs: 2, md: 2 } }}>
                    <StyledPaper elevation={3} sx={{ maxHeight: { md: '90vh' }, height: '100%' }}>
